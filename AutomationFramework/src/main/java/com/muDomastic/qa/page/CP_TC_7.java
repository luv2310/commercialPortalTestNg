...
public class CP_TC_7  {
	...
	public CP_TC_7(WebDriver driver)
	{
	...
	public void TC_07_TS_07() {
		try {
			...
			action.verifyElementPresent(accountName);	
			action.sendText(accountName, "Acc name");

			action.verifyElementPresent(sc1);	
			action.sendText(sc1, "40");

			action.verifyElementPresent(sc2);	
			action.sendText(sc2, "47");

			action.verifyElementPresent(sc3);	
			action.sendText(sc3, "84");

			action.verifyElementPresent(accountNumber);	
			action.sendText(accountNumber, "70872490");

			action.verifyElementPresent(paymentsave);	
			action.clickWithjavascriptattempt(paymentsave);
			...
			{
				String udhjana = ele.getText();
				if(udhjana.contains("Confirm With Email"))
				{
					ele.click();
					break;
				}				
